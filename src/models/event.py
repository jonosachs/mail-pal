...
class Event(BaseModel):
    ...
    start: str = Field(
        description="ISO 8601 format string e.g. 2026-03-22T15:00:00+11:00"
    )
    end: str = Field(
        description="ISO 8601 format string e.g. 2026-03-22T19:00:00+11:00"
    )
    recurrence: Optional[List[str]] = Field(
        default=None,
        description="Format: RRULE:FREQ=;COUNT=, omit field for single event",
    )
    ...
    source_url: Optional[str] = Field(
        default=None, description="https://mail.google.com/mail/u/0/#inbox/{msg_id}"
    )
...
class Events(BaseModel):
    ...
    notes: Optional[str] = Field(
        default=None,
        description="If you decide against returning an event, or return no events, provide brief reasoning",
    )
